...
- **Contras:** amarra o core a um editor; Grok/Claude/humanos ficam de fora; drift entre plugin e `docs/SDD_WORKFLOW.md`.
...
**Descrição:** Status quo. Humano cola `prompts/*.md`.
...
- **Fonte:** `docs/SDD_WORKFLOW.md` §14.2
...
Não adotamos a filosofia pstack “the best spec is code” nem “never block on the human”. Copiamos só o mecanismo. Fonte do recorte: https://github.com/cursor/plugins/blob/main/pstack/README.md (seção “why are there no planning skills?”) vs `docs/SDD_WORKFLOW.md` §1.
...
- Agentes que não leem `.agent/skills/` ainda precisam do ponteiro em `prompts/`.
...
1. Manter `prompts/` utilizáveis como ponteiros (fase 3 não apaga o nome do cenário).
2. Remover `.agent/skills/sdd-mode/` e `principle-*`.
3. Restaurar passos longos em `prompts/` a partir do git.
